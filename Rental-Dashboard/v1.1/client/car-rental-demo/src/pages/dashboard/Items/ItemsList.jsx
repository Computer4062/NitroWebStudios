...
function ItemsList() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
		...
		const usersAdmin = await api.get("/api/accounts/check-auth", {
          withCredentials: true
        });
      	setIsAdmin(usersAdmin.data.admin);
  ...
  const displayedItems = items.filter((item) => {
	...
	// Check if query exists in name, brand, or type
	const matchesSearch = (
	...
	// Days-left filter — only applies to Rented items
	if (filters.expiryWindow !== "any") {
		if (item.status !== "Rented") return false;
		if ((item.days_left ?? Infinity) > Number(filters.expiryWindow)) return false;
	}
			...
			<div className="col-lg-9">
				...
				{displayedItems.length === 0 ? (
					...
					displayedItems.map((item) => {
						const isOwner = isAdmin; // No 'user' column exists on vehicles yet — see note below
						...
						<div className="col-12 col-xl-6" key={item.id}>
							...
							{/* Status Footer */}
							<div className="card-footer bg-light-subtle border-top border-light p-3 d-flex align-items-center justify-content-between">
								{item.status === "Available" ? (
									<span className="badge bg-success-subtle text-success">Available</span>
								) : (
									<span className="badge bg-warning-subtle text-warning">
										Rented &middot; {item.days_left ?? '?'}d left
									</span>
								)}
							</div>
...
function FilterPanelContent({ filters, setFilters, toggleFilterValue, onPriceMinChange, onPriceMaxChange, resetFilters, flush }) {
	...
	const content = (
		<>
			...
			<FilterSection title="Rent Expiring In">
				<select
					className="form-select form-select-sm"
					value={filters.expiryWindow}
					onChange={(e) => setFilters(prev => ({ ...prev, expiryWindow: e.target.value }))}
				>
					{EXPIRY_OPTIONS.map((opt) => (
						<option value={opt.value} key={opt.value}>{opt.label}</option>
					))}
				</select>
				<p className="text-muted small mt-1 mb-0">Only applies to currently rented vehicles.</p>
			</FilterSection>
